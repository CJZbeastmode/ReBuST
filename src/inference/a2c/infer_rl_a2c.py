...
def _infer_recursive(wsi, model, history, lvl, x, y, device, deterministic, verbose=True):
    """
    ...
    if action == 0:
        print(f"[STOP] lvl={lvl} x={x} y={y} "
              f"p_stop={p_stop:.4f} p_zoom={p_zoom:.4f} v={v:.4f}")
        return

    # ZOOM
    print(f"[ZOOM] lvl={lvl} \u2192 {child_level} x={x} y={y} "
          f"p_stop={p_stop:.4f} p_zoom={p_zoom:.4f} v={v:.4f}")
    try:
...
def infer_wsi_a2c(
    ...
    _vprint = print if verbose else lambda *a, **kw: None
    _vprint(f"[INFER] Levels: max={wsi.max_level}  min={wsi.min_level}  "
    ...
    # ------------------------------------------------------------------
        ...
        for idx, (lvl, x, y) in enumerate(zoomed_seeds):
            if idx % 50 == 0:
                _vprint(f"[INFER] {idx}/{total} processed "
                      f"(active={wsi.active_patch_count()} "
                      f"zoomed={len(wsi.zoomed_patches)})")
            ...
            if action == 0:
                _vprint(f"[UNZOOM] lvl={lvl} x={x} y={y} "
                      f"p_stop={p_stop:.4f} p_zoom={p_zoom:.4f} v={v:.4f}")
                _do_unzoom(wsi, lvl, x, y)
            else:
                _vprint(f"[KEEP-ZOOMED] lvl={lvl} x={x} y={y} "
                      f"p_stop={p_stop:.4f} p_zoom={p_zoom:.4f} v={v:.4f}")

    _vprint(f"[INFER] Done — active={wsi.active_patch_count()}  "
          f"zoomed={len(wsi.zoomed_patches)}")

    if output_html is not None:
        mode_tag = "A2C Level-4 (single-stage)" if not wsi.multistage else "HUMBE + A2C Level-4"
        meta_viz = {
            "Method":         mode_tag,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-viz-path", type=str,
                        default="data/visualizations/rl/a2c/viz_a2c.html")
