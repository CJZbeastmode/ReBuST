...
Local Decision Problem (No Budget, No Constraints)

This implementation learns when to stop zooming based on immediate
patch quality scores and predicted future rewards.

Key principles:
---------------
1. NO budget constraints, NO CMDP formulation, NO Lagrange multipliers
2. Pure Actor-Critic with TD advantages
3. IMMEDIATE rewards at each step (patch semantic alignment scores)
4. Critic learns: "What cumulative reward will I get from here?"
5. Actor learns: "Should I zoom or stop at this patch?"

Reward structure:
-----------------
- STOP action → reward = text_alignment_score(current_patch)
- ZOOM action → reward = text_alignment_score(child_patches)
- Rewards reflect LOCAL patch quality (what you want!)

Training objective:
-------------------
- Low-quality patches → STOP early (low reward, no benefit from zooming)
- High-quality patches → Decide based on whether children are better
- Policy learns LOCAL zoom decisions based on embeddings + predicted future

Network architecture:
---------------------
- Shared encoder: state → hidden representation
- Actor head: hidden → logits(STOP, ZOOM)
- Critic head: hidden → expected cumulative future reward V(s)
"""
...
ENTROPY_BETA = 0.03         # Entropy regularization (moderate exploration)
...
class ActorCritic(nn.Module):
    ...
    def __init__(self, state_dim=515, hidden_dim=256):
        ...
        # Increased actor gain to encourage stronger initial policy signal
        nn.init.orthogonal_(self.actor.weight, gain=0.1)
        nn.init.orthogonal_(self.critic.weight, gain=1.0)
        # Initialize actor bias to favor zooming (exploration)
        nn.init.zeros_(self.actor.bias)
        self.actor.bias.data[1] = 0.1  # Small bias toward zoom action
...
def rollout_episode(env, model, device):
    ...
    The environment provides IMMEDIATE rewards at each step based on
    patch semantic alignment scores. This is perfect for learning
    local zoom decisions.
    ...
    trajectory : dict
        ...
        - rewards: list of IMMEDIATE rewards (patch scores at each step)
    ...
    while not done:
        ...
        # CRITICAL: Reward shaping to incentivize intelligent zooming
        # The environment gives raw patch scores, but we need to reward
        # the DECISION quality, not just the patch quality
        if action.item() == 1 and 's_stop' in info and 's_zoom' in info:
            # ZOOM action: reward the GAIN from zooming
            # If children are better, positive reward. If worse, negative.
            s_stop = info['s_stop']
            s_zoom = info['s_zoom']
            if s_zoom is not None and s_stop is not None:
                # Reward = improvement from zooming
                reward = s_zoom - s_stop
            # else: keep original reward
...
def compute_advantages_and_returns(trajectory, gamma=GAMMA):
    """
    Compute returns and advantages using TD(0) with immediate rewards.
    
    This is the CORRECT formulation for immediate-reward MDPs:
    - Returns = sum of discounted immediate rewards
    - Advantages = TD errors using actual step rewards
    
    The environment gives immediate rewards at each step:
    - r_t = patch_score at step t
    - Return G_t = r_t + γ*r_{t+1} + γ²*r_{t+2} + ...
    - Advantage A_t = r_t + γ*V(s_{t+1}) - V(s_t)
    
    This allows the policy to learn LOCAL decisions based on:
    - Current patch quality (in r_t)
    - Expected future quality (in V(s_{t+1}))
...
def a2c_update(model, optimizer, trajectory, gamma=GAMMA, entropy_beta=ENTROPY_BETA, value_coef=VALUE_COEF):
    ...
    # Recompute forward pass for entropy (need current policy distribution)
    ...
    # 1. Policy loss (actor) - maximize expected advantage
    policy_loss = -(log_probs * advantages.detach()).mean()
    
    # 2. Value loss (critic) - CRITICAL: Must train critic!
