"""
Actor-Critic (A2C) Level 4 — ContrastiveTextScore reward
==========================================================

Identical architecture to a2c.py (GAE, HistoryTracker, parent projection),
but the patch reward is driven by ContrastiveTextScore instead of a
generic text-alignment scorer.

ContrastiveTextScore = cos(patch, c_pos) − max_j cos(patch, c_neg_j)
A positive score means the patch is discriminative for the correct cancer
type, not just "pathology-like".

Cancer type is automatically extracted from each WSI filename:
    TCGA-18-4083-LUSC.svs  →  LUSC

The hard-negative set defaults to "pairs" (biologically curated from
CONTRASTIVE_PAIRS in patch_scores.py) which falls back to "all" when no
entry exists for a given cancer type.  Pass --neg-types all  or
--neg-types LUSC,COAD to override.

Usage
-----
    python src/training/rl/a2c/fail/a2c_lvl4_contrastive.py
    python src/training/rl/a2c/fail/a2c_lvl4_contrastive.py \\
...
from src.utils.patch_scores import ContrastiveTextScore
...
class HistoryTracker:
    ...
    def compute_redundancy_score(self, level, x, y, threshold=OVERLAP_THRESHOLD):
        if len(self.visited_locations) == 0:
            return 0.0
        same_level_visits = [(l, vx, vy) for (l, vx, vy) in self.visited_locations if abs(l - level) < 0.1]
        if len(same_level_visits) == 0:
            return 0.0
        distances = [np.sqrt((x - vx)**2 + (y - vy)**2) for (_, vx, vy) in same_level_visits]
...
class ActorCritic(nn.Module):
    def __init__(self, base_state_dim=520, parent_emb_dim=512, parent_proj_dim=64, hidden_dim=512):
        super().__init__()
        self.parent_proj = nn.Sequential(nn.Linear(parent_emb_dim, parent_proj_dim), nn.ReLU())
        final_state_dim = base_state_dim + parent_proj_dim + 1
        self.encoder = nn.Sequential(
            nn.Linear(final_state_dim, hidden_dim),
            nn.LayerNorm(hidden_dim), nn.ReLU(), nn.Dropout(0.1),
    ...
    def forward(self, x):
        ...
        x_proj = torch.cat([base_state, self.parent_proj(parent_emb), has_parent], dim=1)
...
def rollout_episode(env, model, device,
    ...
    while not done:
        ...
        adjusted_reward     = reward - redundancy_penalty * (redundancy_score + overlap_penalty_val)
    ...
    return {
        ...
        'rewards': rewards, 'raw_rewards': raw_rewards,
        'terminal_reward': rewards[-1] if rewards else 0.0,
        'steps': steps, 'info': info, 'history': history,
    }
...
def a2c_update(model, optimizer, trajectory,
    ...
    avg_raw_reward = float(np.mean(trajectory['raw_rewards'])) if trajectory['raw_rewards'] else 0.0
...
def train_a2c(
    ...
    output_dir='data/models/rl/a2c_lvl4_contrastive',
    device=None,
    redundancy_penalty=REDUNDANCY_PENALTY,
    overlap_threshold=OVERLAP_THRESHOLD,
    ...
    print(f"[A2C-Contrastive] Training on device: {device}")
    print(f"neg_types={neg_types} | GAE lambda: {gae_lambda} | Redundancy penalty: {redundancy_penalty}")
    ...
    # Parse neg_types: "all", "pairs", or "LUSC,COAD,..." → list
    ...
    for epoch in range(num_epochs):
        ...
        epoch_metrics = {k: [] for k in ('loss', 'terminal_reward', 'episode_return',
                                          'steps', 'entropy', 'redundancy_impact')}
        ...
        for img_path in image_paths:
            ...
            if not cancer_type:
                print(f"[WARN] Could not extract cancer type from '{img_name}' — skipping.")
                continue

            try:
                wsi = WSI(img_path, embedder=embedder)

                # Build the contrastive scorer for this cancer type
                ...
                print(f"[INFO] {img_name}  cancer_type={cancer_type}  "
                      f"neg_types={scorer.neg_cancer_types}")
                ...
                env.patch_score_module = scorer   # override with per-image contrastive scorer

                for ep in range(episodes_per_image):
                    trajectory = rollout_episode(env, model, device,
                                                 redundancy_penalty, overlap_threshold)
                    metrics = a2c_update(model, optimizer, trajectory, gae_lambda=gae_lambda)
        ...
        ckpt = os.path.join(output_dir, f"a2c_lvl4_contrastive_epoch_{epoch+1}.pt")
        torch.save({
            'epoch': epoch + 1,
    ...
    final_path = os.path.join(output_dir, "a2c_lvl4_contrastive_final.pt")
...
def main():
    parser = argparse.ArgumentParser(
        description="A2C Level 4 — ContrastiveTextScore reward (cancer type auto-detected from filename)"
    )
    parser.add_argument('--images-dir', type=str, default='data/images',
                        help='Directory containing WSI images')
    parser.add_argument('--neg-types', type=str, default='pairs',
                        help=(
                            "Hard-negative set for contrastive scorer: "
                            "'pairs' (default, biologically curated CONTRASTIVE_PAIRS), "
                            "'all' (every other cancer type), "
                            "or comma-separated list e.g. 'LUSC,COAD'."
                        ))
    parser.add_argument('--epochs', type=int, default=10)
    parser.add_argument('--episodes-per-image', type=int, default=5)
    parser.add_argument('--output-dir', type=str,
                        default='data/models/rl/a2c_lvl4_contrastive')
