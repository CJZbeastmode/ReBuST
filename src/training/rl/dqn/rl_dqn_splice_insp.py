# B_qlearning_full.py
"""
SPLICE-Inspired DQN for Dynamic Patch Selection

Based on baseline DQN with SPLICE principles integrated:

SPLICE Core Idea: Keep a patch only if it adds NEW information.
- Q-values approximate *marginal information gain*
- Novelty bonus reward for patches dissimilar to history
- Redundancy penalty for patches similar to previously seen

Key differences from baseline DQN:
- Tracks trajectory memory of visited patches (like PAMIL)
- Novelty-based reward shaping (SPLICE: value = novelty to history)
- Redundancy penalty discourages repetitive zooms
- Interprets Q(s, ZOOM) as "expected marginal information gain"

This is a value-based greedy baseline inspired by SPLICE's
instance selection principle, serving as comparison to A2C/PAMIL.
"""
...
# ============================================================
# SPLICE-Inspired hyperparameters
# ============================================================
# SPLICE Principle: Value patches by marginal information gain
TRAJ_MEM_SIZE = 5           # Track recent patch embeddings for novelty computation
NOVELTY_REWARD_SCALE = 0.15 # Bonus reward for novel patches (SPLICE: keep if new)
REDUNDANCY_PENALTY = 0.10   # Penalty for redundant patches (SPLICE: filter similar)

# ============================================================
# Utility: Cosine Similarity for Novelty Computation
# ============================================================
def cosine_sim(a, b):
    """
    SPLICE-inspired novelty measure.
    Computes cosine similarity between two state embeddings.
    Used to determine if a patch adds new information.
    """
    ...
    def train_step(self, batch_size=64):
        ...
        with torch.no_grad():
            ...
            if len(non_terminal_idx) > 0:
                ...
                q_next_vals = target_q_next.gather(1, best_actions.unsqueeze(1)).squeeze(1)  # (B_nt,)
...
# =========================
# TRAINING LOOP (SPLICE-Inspired)
# =========================

def run_episode_dqn(env, agent, lambda_zoom, device, max_steps, batch_size=64):
    """
    Run one episode with SPLICE-inspired reward shaping.
    
    SPLICE Principle: Q-values should approximate marginal information gain.
    - Track trajectory memory of visited patches
    - Reward novelty (patches dissimilar to history)
    - Penalize redundancy (patches similar to history)
    - Cost remains simple binary (ZOOM=1, STOP=0)
    """
    state = torch.tensor(env.reset(), dtype=torch.float32, device=device)

    ep_reward_raw = 0.0  # Raw environment rewards
    ep_reward_eff = 0.0  # Rewards after constraint and SPLICE shaping
    zoom_count = 0
    steps = 0
    training_steps = 0
    
    # ============================================================
    # SPLICE: Trajectory Memory for Novelty Computation
    # ============================================================
    # Maintains recent patch embeddings to measure redundancy
    # NOT used to enforce stopping (that's lambda's job)
    traj_memory = []
    ...
    for _ in range(max_steps):
        ...
        # ---- Cost definition (same as baseline) ----
        ...
        # ============================================================
        # SPLICE-Inspired Reward Shaping
        # ============================================================
        # Goal: Make Q(s, ZOOM) ≈ expected marginal information gain
        # Method: Add novelty bonus, subtract redundancy penalty
        
        novelty_reward = 0.0
        redundancy_penalty = 0.0
        
        if action == 1 and next_state is not None and not done:
            curr_tensor = torch.tensor(next_state, device=device)
            
            if len(traj_memory) > 0:
                # SPLICE: Compute novelty as (1 - max_similarity)
                # High similarity → low novelty → low value
                # Low similarity → high novelty → high value
                sims = [cosine_sim(curr_tensor, past) for past in traj_memory]
                max_sim = max(sims)
                novelty = 1.0 - max_sim
                
                # SPLICE: Bonus reward for novel patches
                # Encourages agent to seek NEW information
                novelty_reward = NOVELTY_REWARD_SCALE * novelty
                
                # SPLICE: Penalty for redundant patches (high similarity)
                # Discourages zooming on similar patches
                if novelty < 0.3:  # Redundant threshold
                    redundancy_penalty = REDUNDANCY_PENALTY * (0.3 - novelty)
            else:
                # First zoom always has maximum novelty
                novelty = 1.0
                novelty_reward = NOVELTY_REWARD_SCALE * novelty
            
            # Update trajectory memory (FIFO)
            traj_memory.append(curr_tensor.detach())
            if len(traj_memory) > TRAJ_MEM_SIZE:
                traj_memory.pop(0)
        
        # ============================================================
        # Final Effective Reward
        # ============================================================
        # reward_eff = base_reward + novelty_bonus - redundancy_penalty - lambda*cost
        # This makes Q-value approximate: marginal_info_gain - constraint_cost
        reward_eff = reward + novelty_reward - redundancy_penalty - lambda_zoom * cost
...
def main():
        ...
        for img in train_images:
            ...
            for ep in range(args.episodes_per_wsi):
                stats = run_episode_dqn(
                    env, agent, lambda_zoom, device, args.max_steps, batch_size=args.batch_size
                )
    ...
    if args.output:
        os.makedirs(args.output, exist_ok=True)
        path = os.path.join(args.output, "dqn_splice_inspired.pt")
        torch.save(agent.q.state_dict(), path)
        print(f"Model saved to {path}")
