"""Inference for aggregation transformer (ABMIL/CLAM/WiKG pooling)."""
...
def _load_embeddings_and_coords(
    ...
    if isinstance(loaded, dict):
        ...
        if loaded.get("label") is not None:
            label_name = str(loaded.get("label"))
...
def load_model(checkpoint_path: str, device: torch.device) -> Tuple[AggregationTransformer | PureMIL, Dict[str, int], Dict]:
    ...
    if cfg.get("model_type") == "pure_mil":
        model = PureMIL(
            ...
            pooling_type=cfg.get("pooling_type", "abmil"),
        ).to(device)
    else:
        model = AggregationTransformer(
            ...
            attn_dim=cfg.get("attn_dim", 128),
            pooling_type=cfg.get("pooling_type", "abmil"),
            wikg_k=cfg.get("wikg_k", 16),
            wikg_steps=cfg.get("wikg_steps", 2),
            wikg_temperature=cfg.get("wikg_temperature", 1.0),
        ).to(device)

    # strict=False: old checkpoints lack logit_bias buffer (defaults to zeros = no adjustment)
...
def predict_one(
    ...
    attn = extras.get("attention")
    if attn is not None:
        attn_values = attn[0].detach().cpu().numpy().tolist()
    else:
        attn_values = []

    return {"pred": pred, "probs": probs.tolist(), "attention": attn_values}
...
def main(args):
    ...
    model, label_map, cfg = load_model(checkpoint_path, device)
    ...
    if args.input_pt:
        ...
        for fname in pt_files:
            ...
            embs, coords, label_name = _load_embeddings_and_coords(pt_path, max_patches)
            out = predict_one(model, embs, coords, device)
            predictions.append(
                {
                    "case_id": case_id,
    ...
    if out_json_path is None:
        out_json_path = os.path.join(
            "data",
            "benchmark",
            f"aggregation_transformer_{method.lower()}.json",
        )

    if args.output and out_json_path:
...
def parse_args():
    ...
    p.add_argument("--max-patches-per-wsi", type=int, default=0)
    p.add_argument(
        "--bootstrap-samples",
        type=int,
        default=1000,
