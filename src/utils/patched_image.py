...
class PatchedImage:
    """
    Represents a crawled / patched image as a forest of PatchNodes.

    - Constructed from an explicit list of patch coordinates
    - Only patches in `patch_coords` exist
    - Hierarchy is reconstructed using WSI geometry
    - Roots are stored explicitly
    ...
    def generate_visualization(self, output_html="./data/visualizations/temp2.html"):
        ...
        zoomed = self.zoomed_coords

        all_patches_count = 0
        for lvl, info in self.wsi.levels_info.items():
        ...
        # -------------------------------------------------------------------------
        thumb_level = self.wsi.max_level
        thumb_w, thumb_h = self.wsi.levels_info[thumb_level]["size"]

        # One pixel in the thumbnail is one pixel at thumb_level
        thumbnail = self.wsi.slide.read_region((0, 0), thumb_level, (thumb_w, thumb_h))
        ...
        # -------------------------------------------------------------------------
        level0_w, _ = self.wsi.levels_info[0]["size"]
        ...
        for lvl, x, y in kept:
            ...
            X, Y, S = to_thumb(lvl, x, y, self.wsi.patch_size)
