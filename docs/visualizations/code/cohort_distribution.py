...
ax.axhline(
    y=rare_y_max, color="#e67e22", linewidth=1.2, linestyle="--", zorder=4, alpha=0.8
)

bars = ax.barh(y_pos, percents_s, height=0.62, color=colors, edgecolor="none", zorder=3)
...
ax.annotate(
    "",
    xy=(bracket_x, rare_y_min + 0.1),
    xytext=(bracket_x, rare_y_max - 0.1),
    arrowprops=dict(arrowstyle="]-[", color="#e67e22", lw=1.4, mutation_scale=6),
)
ax.text(
    bracket_x + 0.3,
    (rare_y_min + rare_y_max) / 2,
    "Rare\nclasses\n(21.4%)",
    va="center",
    ha="left",
    fontsize=8.5,
...
plt.savefig("cohort_distribution.png", dpi=180, bbox_inches="tight", facecolor="white")
plt.show()
